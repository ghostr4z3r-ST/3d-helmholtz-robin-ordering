...
def run_case(nx: int, ny: int, nz: int, beta: float, shear_xy: float, modes: int, radius: float,
             Lx: float = 1.0, Ly: float = 1.0, Lz: float = 1.0) -> dict:
    corners = make_corners('hexahedron', Lx=Lx, Ly=Ly, Lz=Lz, shear_xy=shear_xy, shear_xz=0.0, shear_yz=0.0)
    ...
    face_sig = local_octant_signature(mesh, vecs[:, best_idx], logical_center=locs['face_x0'], radius=radius)
    edge_sig = local_octant_signature(mesh, vecs[:, best_idx], logical_center=locs['edge_x0y0'], radius=radius)
    corner_sig = local_octant_signature(mesh, vecs[:, best_idx], logical_center=locs['corner_near'], radius=radius)
    ...
    return {
        ...
        'center_top1_coeff': float(center_sig['ranked'][0][1]),
        'center_top2': center_sig['ranked'][1][0],
        'center_top2_coeff': float(center_sig['ranked'][1][1]),
    }


def main() -> None:
    ap = argparse.ArgumentParser(description='Shear-family study for the hexahedral carrier class.')
    ...
    ap.add_argument('--betas', type=str, default='1,4.8')
